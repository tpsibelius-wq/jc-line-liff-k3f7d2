// 沼津JC 公式LINE の画面（参加者・会員・管理者）。設定は config.js、見た目は styles.css
var LIFF_ID = (window.JC_CONFIG || {}).LIFF_ID || "";
var API = (window.JC_CONFIG || {}).API || "";

var TOKEN = "", STATE = null, TARGET = "", MODE = "user", VIEW = "", VIEW_DONE = false, A_IMG = "", A_RATIO = "", COPY_FROM = "";
var DBG = {};
function $(id){ return document.getElementById(id); }
var TODO_SCOPE = (function(){ try { return localStorage.getItem("jc_todo_scope") || "mine"; } catch (e) { return "mine"; } })();
var TOAST_T = null;
...
function shareEvent(e){
  var text = "【沼津JC】" + e.date + " " + e.name + (e.place ? "\n📍 " + e.place : "") + (e.liff ? "\n申込はこちら: " + e.liff : "");
...
function render(st){
  ...
  fillSelect("f_gyoshu", st.gyoshuList, pf0.gyoshu);
  fillSelect("f_kikkake", st.keiroList, pf0.kikkake);
  $("f_name").value = pf0.name || $("f_name").value;
  $("f_company").value = pf0.company || $("f_company").value;
...
function renderMemberView(st){
  ...
  if (st.isMember){
    ...
    if (b && b.top && b.top.length){
      rl.appendChild(el("div", "t", "🏅 紹介ランキング（候補者／入会）"));
      b.top.forEach(function(r, i){ rl.appendChild(el("div", "", (i + 1) + ". " + r.name + " " + r.count + "人／入会 " + r.joined)); });
      if (b.myRank) rl.appendChild(el("small", "", "あなた: " + b.myRank + "位（候補者 " + b.mine.count + "人／入会 " + b.mine.joined + "）"));
      else rl.appendChild(el("small", "", "あなたの紹介はまだ集計されていません。申込のとき「ご紹介者」にあなたの名前を入れてもらうと数えられます"));
    }
  } else {
    rl.style.display = "none";
    rp.style.display = "none"; // 紹介は会員の機能。未入会者には見せない
  }
  if (VIEW && !VIEW_DONE){
    VIEW_DONE = true;
    if (VIEW === "qr") showMyQr("myqr");
    if (VIEW === "share") setTimeout(shareReferral, 300);
    if (VIEW === "survey" && st.isMember){ $("survey").style.display = "block"; setTimeout(function(){ $("survey").scrollIntoView({ behavior: "smooth", block: "start" }); }, 100); return; }
    setTimeout(function(){ rp.scrollIntoView({ behavior: "smooth", block: "start" }); }, 100);
  }
}

// ---- 入会後アンケート（会員画面・?v=survey で開く）----
function pickScore(v, btn){
  $("sv_score").value = String(v);
  Array.prototype.forEach.call($("sv_btns").children, function(b){ b.style.background = ""; b.style.color = ""; });
...
function handoffText(c){
  var lines = ["【新入会員 引継ぎ】" + c.name + (c.company ? "（" + c.company + "）" : ""),
...
function showIntro(force){
  ...
  try { if (!force && localStorage.getItem("jc_intro_done")) return; } catch (e) {}
  try { localStorage.setItem("jc_intro_done", "1"); } catch (e) {}
...
// 担当ごとの進行中の件数（入会・辞退以外）。割り当てのときに負荷を見るため
function assigneeCounts(){
  var m = {}; ((STATE && STATE.all) || []).forEach(function(c){ if (c.tanto && !/^(7 |辞退)/.test(String(c.status || ""))) m[c.tanto] = (m[c.tanto] || 0) + 1; });
  return m;
}
...
function todoCard(c){
  ...
  var head = el("div", "", c.name + (c.company ? "（" + c.company + "）" : "")); head.style.fontWeight = "bold"; card.appendChild(head);
  card.appendChild(el("small", "", "次: " + c.next + due + " ／ 担当: " + (c.tanto || "なし") + (c.hasLine ? "" : " ／ LINE未紐付")));
  if (c.handoff) card.appendChild(el("small", "", "🎉 入会後の手順 " + c.handoff.step + "/" + c.handoff.total));
  var row = el("div", "apl");
...
function renderCases(st){
  ...
  if (sc){ sc.innerHTML = "";
    if (me) [["mine", "自分の担当＋担当なし", mine.filter(isDue).length], ["all", "全員分", all.filter(isDue).length]].forEach(function(o){
      ...
      b.onclick = function(){ TODO_SCOPE = o[0]; try { localStorage.setItem("jc_todo_scope", TODO_SCOPE); } catch (e) {} renderCases(STATE); };
      sc.appendChild(b);
    });
  }
  ...
  if (!now.length){
    ...
    var un = (st.all || []).filter(function(c){ return !c.tanto && !/^(7 |辞退)/.test(String(c.status || "")); }).length;
    if (un){ var b1 = el("button", "b_sub", "担当なしの候補者 " + un + " 人を見る"); b1.onclick = function(){ showView("cases"); var t = $("a_tanto"); if (t){ t.value = "__none__"; } renderAll(); }; emp.appendChild(b1); }
    box.appendChild(emp);
  }
...
// ---- 候補者一覧（名簿・1行） ----
var LAST_ROSTER = [];
// 委員会用の一覧を印刷（いまの絞り込みのまま。連絡先は載せない）
function printCases(){
  var list = LAST_ROSTER || [];
  if (!list.length){ alert("表示中の候補者がいません"); return; }
...
function renderAll(){
  ...
  var list = (STATE.all || []).filter(function(c){
    ...
    if (!q) return true;
    return [c.name, c.company, c.tanto, c.gyoshu, c.kikkake, c.shokai].join(" ").toLowerCase().indexOf(q) >= 0;
  });
  var ch = $("stage_chips");
  if (ch){ ch.innerHTML = "";
  ...
  LAST_ROSTER = list;
  if (!list.length){ box.appendChild(el("div", "hint", q ? "該当なし（未登録なら「＋ 追加」）" : "まだ候補者がいません")); return; }
  ...
  list.slice(0, 200).forEach(function(c){
    ...
    var nm = el("div", "nm", c.name + (c.company ? "（" + c.company + "）" : ""));
...
function rosterTable(list){
  var cols = [["name", "名前"], ["company", "会社"], ["status", "段階"], ["tanto", "担当"], ["next", "次にやること"], ["due", "期限"], ["updated", "更新"]];
  ...
  sorted.slice(0, 500).forEach(function(c){
    ...
    [c.name, c.company || "", c.status, c.tanto || "", c.next || "", due, relDate(c.updatedTs, c.updated)].forEach(function(v, i){
...
function openDetail(id){
  ...
  $("cd_info").innerHTML = "";
  if (isNew){
    $("cd_title").textContent = "候補者を追加";
    fillSelect("n_gyoshu", STATE.gyoshuList || [], ""); fillSelect("n_kikkake", STATE.keiroList || [], "");
  } else {
    fillDetail(id);
  }
...
function fillDetail(id){
  ...
  $("cd_title").textContent = c.name + (c.company ? "（" + c.company + "）" : "") + "　" + c.id;
  ...
  var attrs = [c.gyoshu, c.yakushoku, c.nenrei, c.area, c.kanshin].filter(String).join("・"); if (attrs) line("属性: " + attrs);
  if (c.contact){
  ...
  if (c.handoff) line("🎉 入会後の手順 " + c.handoff.step + "/" + c.handoff.total + "（「対応済み」で次へ）");
  if (c.partAfter !== null && c.partAfter !== undefined) line("入会後のイベント参加 " + c.partAfter + "回");
  ...
  // その他
  $("cd_name").value = c.name; $("cd_company").value = c.company || "";
  fillSelect("at_gyoshu", STATE.gyoshuList || [], ""); $("at_gyoshu").value = c.gyoshu || "";
  fillSelect("at_nenrei", STATE.nenreiList || [], ""); $("at_nenrei").value = c.nenrei || "";
  ...
  if (c.hasLine && !/^7 /.test(c.status)) add("会員にする", "b_att", function(){ if (confirm(c.name + " を会員にしますか？（段階は「7 入会」、メニューが会員用に切り替わります）")) caseOp(c.id, "to_member", ""); });
  if (!/^(7 |辞退)/.test(c.status)) add("面談を記録", "b_att", function(){ recordMeeting(c); });
  if (!c.hasLine) add("紹介文をLINEで送る", "", function(){ shareReferral(); });
  if (/^[567] /.test(c.status)) add("引継ぎ文をコピー", "", function(){ copyText(handoffText(c)); });
  if (/^7 /.test(c.status)) add("総務へ送る", "", function(){ if (confirm("総務担当に引継ぎ文をLINEで送ります。よろしいですか？")) { say("送信中…"); api("liff_admin_handoff", { id: c.id }).then(renderAdmin).catch(function(e){ say("エラー: " + e.message); }); } });
  add("統合", "", function(){ var t = prompt("残す側の番号（例 C-003）。この人（" + c.id + "）は「辞退・対象外」になり、空欄の情報は残す側に寄せます"); if (!t) return; if (confirm(c.id + " を " + t.trim() + " に統合しますか？")) caseOp(c.id, "merge", t.trim()); });
  add("削除", "b_abs", function(){
    var ev = (c.events && c.events.length) || 0;
    var warn = c.name + " を候補者リストから削除します。" + (ev ? "この人の申込 " + ev + " 件の記録も消えます（イベントの人数・出欠から外れます）。" : "") + "元に戻せません。";
    if (!/^1 /.test(c.status) || ev) warn += "\n実際に辞退した人は「辞退・対象外」にして記録を残すのが基本です。";
    if (confirm(warn) && confirm("本当に削除しますか？")) caseOp(c.id, "delete", "force");
  });
}
// 面談を記録: 所感をメモに残し、段階を「4 面談済み」に、次にやることと期限（7日後）を1回で
function recordMeeting(c){
  var note = prompt("面談の所感（メモに残ります。相手の関心・懸念・約束したことなど）", "");
  if (note === null) return;
  ...
  say("記録中…");
  var memoP = note.trim() ? api("liff_admin_case", { id: c.id, op: "memo", value: "面談: " + note.trim() }) : Promise.resolve(null);
  memoP.then(function(){ return api("liff_admin_case", { id: c.id, op: "update", value: JSON.stringify({ status: /^[4567] /.test(c.status) ? undefined : "4 面談済み", next: nextTxt.trim(), due: due }) }); })
    .then(function(st){ renderAdmin(st); say("面談を記録しました（段階4・次にやること・期限7日後）"); })
    .catch(function(e){ say("エラー: " + e.message); });
}
function saveDetail(){
  ...
  if (up.status && /^辞退/.test(up.status) && !confirm(c.name + " を「辞退・対象外」にしますか？（追撃はしません）")) return;
...
function saveOther(){
  var c = caseById(DETAIL_ID); if (!c) return;
  var n = $("cd_name").value.trim(), co = $("cd_company").value.trim();
  var at = { gyoshu: $("at_gyoshu").value, yakushoku: $("at_yakushoku").value.trim(), nenrei: $("at_nenrei").value, area: $("at_area").value.trim(), kanshin: $("at_kanshin").value };
  var doAttrs = function(){ caseOp(c.id, "attrs", JSON.stringify(at)); };
  if (n !== c.name || co !== (c.company || "")) {
    say("更新中…");
    api("liff_admin_case", { id: c.id, op: "rename", value: JSON.stringify({ name: n, company: co }) }).then(function(st){ renderAdmin(st); doAttrs(); }).catch(function(e){ say("エラー: " + e.message); });
  } else doAttrs();
}
function adminAddCase(force){
  var d = { name: $("n_name").value.trim(), company: $("n_company").value.trim(), gyoshu: $("n_gyoshu").value, kikkake: $("n_kikkake").value,
  ...
  api("liff_admin_add_case", { data: d }).then(function(st){
    ["n_name","n_company","n_shokai","n_memo","n_next"].forEach(function(id){ $(id).value = ""; });
    $("n_gyoshu").value = ""; $("n_kikkake").value = "";
    closeDetail(); renderAdmin(st);
  }).catch(function(e){
    if (!force && /同名の候補者がいます/.test(e.message)){ if (confirm(e.message + "\n\n別人として追加しますか？")) adminAddCase(true); else say(e.message); }
...
function renderMembers(st){
  ...
  list.forEach(function(m){
    ...
    if (refs.length) nm.appendChild(el("span", "tag att", "紹介 " + refs.length + "人（入会 " + refs.filter(function(c){ return /^7 /.test(c.status); }).length + "）"));
    row.appendChild(nm);
    var b1 = el("button", "", "氏名を修正"); b1.onclick = function(){ var n = prompt("氏名（候補者の「ご紹介者」欄と照合します）", m.name || ""); if (n !== null && n.trim()) memberOp("rename", m.uid, n.trim()); }; row.appendChild(b1);
...
function adminMemberMessage(){
  var t = $("a_mmsg").value.trim();
  if (!t){ alert("本文を入力してください"); return; }
  var n = (STATE && STATE.memberCount) || 0;
  if (!confirm("会員" + n + "人に送ります（" + n + "通）。よろしいですか？")) return;
  say("送信中…");
  api("liff_admin_member_message", { text: t }).then(function(st){ $("a_mmsg").value = ""; renderAdmin(st); }).catch(function(e){ say("エラー: " + e.message); });
}
...
function renderKpi(st){
  ...
  var bar = function(label, a, b){
    ...
    var track = el("div", ""); track.style.cssText = "height:8px;background:#e6ecf2;border-radius:4px;overflow:hidden;margin:2px 0 8px";
    var fill = el("div", ""); fill.style.cssText = "height:8px;width:" + p + "%;background:" + (p >= 100 ? "#2E7D32" : "var(--blue)");
  ...
  if (k.newMembers) box.appendChild(el("div", "", "🌱 " + (k.year || "今年") + "年の新会員 " + k.newMembers + "人、うち入会後にイベント参加 " + k.settled + "人"));
  box.appendChild(el("div", "t", "🏅 紹介実績（候補者／面談／入会）"));
  ...
  more.appendChild(el("div", "t", "🚪 きっかけ別（候補者／面談／入会）"));
...
// ---- 文面（3つの群・その場で編集・見本つき） ----
var TEXT_TITLE = {
  "友だち追加": "友だち追加のあいさつ", "即応": "担当が付いたときの一言（本人へ）", "紹介文": "会員が友人に送る紹介文", "入会歓迎": "入会の歓迎",
  "入会後アンケート": "入会後アンケートの案内", "委員会へ連絡": "「委員会へ連絡」への返答", "自由文の返答": "分類できないメッセージへの返答",
...
var TEXT_DESC = {
  ...
  "紹介文": "会員が友人に送る紹介文。末尾に本人専用のリンクが自動で付く", "入会歓迎": "「会員にする」の瞬間に本人へ",
  "入会後アンケート": "入会45日後に本人へ。{liff}にフォームのリンク（無ければ末尾に付く）", "委員会へ連絡": "会員メニューの「委員会へ連絡」への返答",
  "自由文の返答": "自動応答に当てはまらないメッセージへの返答（委員には転送せず、チャットで対応）",
...
function sampleCtx(){
  return { name: "山田太郎", admin: myName() || "（担当名）", event: "10月交流会", date: "10月15日（木）", time: "19:00〜21:00", place: "沼津市内の会場", fee: "3,000円", map: "https://maps.app.goo.gl/xxxx", liff: "https://liff.line.me/…（申込ページ）", info: "📅 10月15日（木） 19:00〜21:00\n📍 沼津市内の会場" };
}
function renderTexts(st){
  ...
  groups.forEach(function(g){
    ...
    items.forEach(function(t){
      ...
      if (g[0] === "auto" && t.notify) head.appendChild(el("span", "tag man", "委員に通知"));
      if (/委員会で確認して入力/.test(t.text)) head.appendChild(el("span", "tag cxl", "仮の本文・未掲載"));
...
function textEditor(t){
  ...
  var pv = el("div", ""); pv.id = "t_preview"; pv.style.cssText = "white-space:pre-wrap;background:#f3f6f9;border-radius:8px;padding:10px;font-size:14px;margin:4px 0 8px";
  wrap.appendChild(pv);
  ...
  if (group === "auto"){
    ...
    var l2 = el("label", ""); l2.style.fontWeight = "normal"; var c2 = document.createElement("input"); c2.type = "checkbox"; c2.id = "t_notify"; c2.style.width = "auto"; c2.checked = isNew ? false : t.notify; l2.appendChild(c2); l2.appendChild(document.createTextNode(" 送られたら委員に知らせる（重要通知の送り先へ）")); wrap.appendChild(l2);
  }
  ...
  mk("保存", function(){ saveText(isNew ? "" : t.key); }, "background:var(--blue);color:#fff");
...
function renderAdmin(st){
  ...
  document.title = "管理画面 - 沼津JC公式LINE";
  ...
  var fi = st.insight || {};
  if (fi.followers !== null && fi.followers !== undefined) lines.push("👥 友だち " + fi.followers + "人（ブロック " + fi.blocks + "・" + fi.date + "時点）うち会員・委員 " + (st.memberCount || 0) + "人 → 未入会者 " + Math.max(0, fi.followers - (st.memberCount || 0)) + "人");
  ...
  renderApplicants();
  if (!VIEW_CUR){
    VIEW_CUR = { cases: "todo", roster: "cases", kpi: "stats", event: "event", texts: "texts", members: "members", todo: "todo", stats: "stats" }[VIEW] || "todo";
  }
  showView(VIEW_CUR);
...
function renderApplicants(){
  ...
  var shown = e.applicants.filter(function(a){ return !q || (a.name + " " + (a.company || "")).toLowerCase().indexOf(q) >= 0; });
